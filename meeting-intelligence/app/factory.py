...
from app.retrieval.reranker import CohereReranker, NoopReranker
...
class Services:
    ...
    def query(self, question: str, meeting_id: str | None = None) -> Answer:
        timings: dict[str, float] = {}
        with timed(timings, "total_ms"):
            chunks = self.retriever.retrieve(question, meeting_id, timings)
            answer = self.answerer.answer(question, chunks, timings)
        log_event(
            logger,
            "query",
            question=question,
...
def build_services(settings: Settings) -> Services:
    ...
    return Services(
        ...
        voice_transcriber=PlainTextTranscriber(),
        vector_store=store,
        ingestion=IngestionPipeline(settings, embedder, store),
